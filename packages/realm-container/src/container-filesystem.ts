...
/**
 * ContainerFilesystem — manages file import/export between host and container.
 */
export class ContainerFilesystem {
  constructor(private readonly docker: Docker) {}

  async importFile(containerId: string, sourcePath: string, destPath: string): Promise<FileRef> {
    ...
    // Read the source file and create a minimal tar in memory
  ...
  async exportFile(containerId: string, sourcePath: string, destPath: string): Promise<FileRef> {
    const container = this.docker.getContainer(containerId);
  ...
  async listFiles(containerId: string, dirPath: string): Promise<FileRef[]> {
    const container = this.docker.getContainer(containerId);
